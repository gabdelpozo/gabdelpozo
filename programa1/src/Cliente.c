...
int cli_modificarArray(Cliente* array,int limite, int indice)
{
	...
	if(array != NULL && limite > 0 && indice < limite && indice >= 0 && array[indice].isEmpty == 0)
	{
		if(	utn_getNombre(bufferCliente.nombre,NOMBRE_LEN,"\nEscriba nombre a modificar: ","\nERROR\n",2) == 0 &&
			utn_getDescripcion(bufferCliente.apellido,APELLIDO_LEN,"\nEscriba apellido a modificar: ","\nERROR\n",2) == 0 &&
			utn_getDni(bufferCliente.cuit,CUIT_LEN,"\nEscriba cuit a modificar: ", "\nError", 2) == 0)
		{
			respuesta = 0;
 ...
 */
/*
int cli_ordenarPorNombrePrecio(Cliente* array,int limite)
{
	int respuesta = -1;
	int flagSwap;
	int i;
